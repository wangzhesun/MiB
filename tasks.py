tasks_coco = {
    "split3": {
        0: [-1, 0, 1, 2, 3, 5, 6, 7, 9, 10, 11, 13, 14, 15, 17, 18, 19, 21, 22, 23, 25, 26, 27, 29, 30,
...
def get_task_labels(dataset, name, step):
    ...
    # if step != 0:
    #     labels_old = list(task_dict[0])
    # else:
    #     labels_old = []
    # labels_old_class = [label for s in range(step) for label in task_dict[s]]
    # num_class = len(labels_old_class + labels)
    # return labels, labels_old, f'data/{dataset}/{name}', num_class
    # ###################################################################################
